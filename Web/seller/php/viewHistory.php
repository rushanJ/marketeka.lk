<?php
 //   session_start();
$id=$_SESSION["shopId"];
    $sql = "SELECT distinct(`orders`.`trolleyId`) FROM `orders`,`item`,`shop` WHERE `orders`.`item`=`item`.`id` AND `item`.`shop`=`shop`.`id` AND( `orders`.`status`= 'CONFIRMED' OR `orders`.`status`= 'REJECTED')";
